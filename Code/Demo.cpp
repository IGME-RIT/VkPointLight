...
void Demo::prepare_sampler()
{
	...
	// We don't have anisotropy, so the max has to be set to 1
...
void Demo::prepare_descriptor_layout()
{
	...
	// In the Fragment Shader, at binding 1, we have 1 descriptor, which is an image
...
void Demo::prepare_descriptor_pool()
{
	...
	// we will have four uniform buffers in the entire program
	type_counts[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	type_counts[0].descriptorCount = 4;

	// we will have four image sampler in the entire program
	type_counts[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	type_counts[1].descriptorCount = 4;
...
void Demo::build_secondary_cmd()
{
	...
	// utilize this feature, the image might look squished or stretched. To fix this, go back to 
	// glm::perspective and change the aspect ratio to match the ratio of the  viewport. If you do not 
	// know what glm::perspective is that comes in a future tutorial, and that means I accidentally
	// left the comment about glm::persepctive here by mistake
	VkViewport viewport = {};
	viewport.x = 0.0f;
	viewport.y = 0.0f;
	...
	// Bonus trick that you can try on your own
	//===============================================================

	// Exzap, from Cemu, has a trick where he adjusts for Vulkan's inverted Y-axis by flipping
	// the viewport, rather than altering the projection matrix. The reason why I personally don't use 
	// this trick here, is because it requires Vulkan 1.1, or it requires Vulkan 1.0 to use the 
	// extension VK_KHR_MAINTENANCE1_EXTENSION_NAME. There is nothing wrong with adding the extension, 
	// nor is there anything wrong with Vulkan 1.1, but for these simple tutorials, I want to make sure
	// that nobody has compatibility problems. Anyone who has a GPU that supports Vulkan 1.0 should
	// be able to run this tutorial, with only the SURFACE_EXTENSION and WIN32_EXTENSION

	// However, I do use Vulkan 1.1 in other personal projects, and I flip the viewport in those projects.
	// If anyone wants to try this trick for themselves, go back to the prepare_instance() function
	// and enable the extension in Vulkan 1.0, or try enabling Vulkan 1.1, then delete the adjustment to the 
	// perspective projection (in update_uniform_buffer, and in prepare_uniform_buffer), then flip 
	// the Viewport here in this function. All Vulkan 1.0 maintenance features are in Vulkan 1.1
	...
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeBasic->pipeline);

	catEntity->Draw(cmd, pipeBasic->pipeline_layout);
	dogEntity->Draw(cmd, pipeBasic->pipeline_layout);
	buildingEntity->Draw(cmd, pipeBasic->pipeline_layout);

	// Bind our pipeline, let Vulkan know that it is a GRAPHICS pipeline.
	// There are other types of pipelines, so we need to specify GRAPHICS.
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeSky->pipeline);

	skyEntity->Draw(cmd, pipeSky->pipeline_layout);
...
void Demo::prepare()
{
	...
	if (firstInit)
	{
		...
		catMesh = new Mesh("../../../Assets/kitten.3Dobj", this);
		dogMesh = new Mesh("../../../Assets/dog.3Dobj", this);
		rockMesh = new Mesh("../../../Assets/building.3Dobj", this);
		skyMesh = new Mesh("../../../Assets/skybox.3Dobj", this);
		...
		catEntity = new Entity(catMesh, catTex, this);
		dogEntity = new Entity(dogMesh, dogTex, this);

		// this is the building from previous OpenGL tutorials
		buildingEntity = new Entity(rockMesh, rockTex, this);
		
		// the skybox has a cube with a 2D PNG, not a cubemap
		skyEntity = new Entity(skyMesh, skyTex, this);
	}
	...
	if (firstInit)
	{
		// if this is our first time loading,
		// we cand delete the CPU buffers that
		// we built. Now that they are copied to
		// the GPU, we don't need them on the CPU
		// anymore

		// Vertex Data
		delete catMesh->vertexDataCPU;
		delete dogMesh->vertexDataCPU;
		delete rockMesh->vertexDataCPU;
		delete skyMesh->vertexDataCPU;

		// Index Data
		delete catMesh->indexDataCPU;
		delete dogMesh->indexDataCPU;
		delete rockMesh->indexDataCPU;
		delete skyMesh->indexDataCPU;

		// Textures
		delete catTex->textureCPU;
		delete dogTex->textureCPU;
		delete rockTex->textureCPU;
		delete skyTex->textureCPU;
	...
	if (firstInit)
	{
		...
		// Blending, etc.
		pipeBasic = new PipeBasic(device, desc_layout, render_pass);
		pipeSky = new PipeSky(device, desc_layout, render_pass);
	}
...
void Demo::draw()
{
	...
	view_matrix = glm::lookAt(cameraPosition, focus, up);	// build the view matrix from thsi data
	...
	catEntity->Update(projection_matrix, view_matrix);
	dogEntity->Update(projection_matrix, view_matrix);
	buildingEntity->Update(projection_matrix, view_matrix);
	skyEntity->Update(projection_matrix, view_matrix);
...
Demo::~Demo()
{
	...
	// Vertex Data
	delete catMesh->vertexDataGPU;
	delete dogMesh->vertexDataGPU;
	delete rockMesh->vertexDataGPU;
	delete skyMesh->vertexDataGPU;

	// Index Data
	delete catMesh->indexDataGPU;
	delete dogMesh->indexDataGPU;
	delete rockMesh->indexDataGPU;
	delete skyMesh->indexDataGPU;

	// Textures
	delete catTex->textureGPU;
	delete dogTex->textureGPU;
	delete rockTex->textureGPU;
	delete skyTex->textureGPU;
